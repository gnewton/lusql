...
# ID is capitalised because derby returns it as ID although it was created as "id"
	...
	time java ca.gnewton.lusql.core.LuSqlMain -so ca.gnewton.lusql.driver.faux.IntegerDocumentDocSource -I ANALYZED:YES:WITH_POSITIONS_OFFSETS  -n ${n} -si ca.gnewton.lusql.driver.http.HttpDocSink ${verbose} &
	sleep 5
	echo "Starting http source 1"
	time java ca.gnewton.lusql.core.LuSqlMain -so ca.gnewton.lusql.driver.http.HttpDocSource  -n ${n} ${verbose} -l luceneHttp1& 
	echo "Starting http source 2"
	time java ca.gnewton.lusql.core.LuSqlMain -so ca.gnewton.lusql.driver.http.HttpDocSource  -n ${n} ${verbose} -l luceneHttp2& 
	wait
